...
public class Problem11 {
	public static void main(String[] args){
		...
		for(int i = 3; i < 20; i++)
			for(int j = 0; j < 20; j++)
				if(nums[i][j] * nums[i-1][j] * nums[i-2][j] * nums[i-3][j] > max)
					max = (nums[i][j] * nums[i-1][j] * nums[i-2][j] * nums[i-3][j]);
		
		//horizontally
		for(int j = 3; j < 20; j++)
			for(int i = 0; i < 20; i++)
				if(nums[i][j] * nums[i][j-1] * nums[i][j-2] * nums[i][j-3] > max)
					max = (nums[i][j] * nums[i][j-1] * nums[i][j-2] * nums[i][j-3]);

		//down and to the right
		for(int i = 3; i < 20; i++)
			for(int j = 3; j < 20; j++)
				if(nums[i][j] * nums[i-1][j-1] * nums[i-2][j-2] * nums[i-3][j-3] > max)
